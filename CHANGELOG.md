...
### Fixed
- **Shop plugin table dependencies**: Added table existence checks in ShopController
  - Orders, shop_items, and shop_categories tables now checked before queries
  - Fixes "no such table" errors when Shop plugin migrations haven't run
  - Dashboard returns 0 for missing tables instead of crashing
...
### Fixed
- **Permission creation during installation**: Added role_id to Permission model fillable attributes
  - Fixes "NOT NULL constraint failed: permissions.role_id" error
  - Admin permissions can now be created during web installation

## [1.4.1] - 2026-02-08

### Fixed
- **Web installer failure**: Removed --seed flag that caused ShopSeeder error
- **Admin role creation in installer**: Added admin role and permissions creation to web installer
  - Installer now creates admin role independently from seeders
  - Fixes "These credentials do not match our records" after installation
